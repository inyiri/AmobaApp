...
{
//    Image pirosGyoztesSzinek[] = new Image[5];
//    Image zoldGyoztesSzinek[]  = new Image[5];
    ...
    HTML html = new HTML("<img src='./imi_fiu.jpg' > <b>Zsazsának és Imrusnak</b>");
...
//        for (int i=0; i<5; i++)
//        {
//            pirosGyoztesSzinek[i] = new Image("red.gif");
//            pirosGyoztesSzinek[i].setSize("15", "15");
//            zoldGyoztesSzinek[i] = new Image("green.gif");
//            zoldGyoztesSzinek[i].setSize("15", "15");
//        }
        ...
        //
        // Nulláza a tabellát 0 : 0 -ra
        //
        btnNullaz.addClickListener(new ClickListener()
        {
    ...
    {
        //amobaView.cells[s][o].setEnabled(false);

        if ( TSzin.KEK == color )
        {
            //amobaView.cells[s][o].getUpFace().setImage( amobaView.imgs.greenKor().createImage() );
            //amobaView.cells[s][o].getUpFace().setImage( amobaView.zoldImage[s][o] );
            amobaView.cells[s-1][o-1].setUrl("green.jpg");
        }
        else
        {
            //amobaView.cells[s][o].getUpFace().setImage( amobaView.imgs.redKor().createImage() );
            //amobaView.cells[s][o].getUpFace().setImage( amobaView.pirosImage[s][o] );
            amobaView.cells[s-1][o-1].setUrl("red.jpg");
        }
    ...
    public void vizualisGyoztesVonal()
    {
        ...
        if (amobaModell.amoba.palya.Get(amobaModell.amoba.xGyoztes, amobaModell.amoba.yGyoztes) == TSzin.PIROS)
        {
            ...
            for (i = 1; i <= 5; i++)
            {
               //amobaView.cells[yKezdo + (i - 1) * dy][xKezdo + (i - 1) * dx].getUpFace().setImage(pirosGyoztesSzinek[i - 1]);
               amobaView.cells[yKezdo-1 + (i - 1) * dy][xKezdo-1 + (i - 1) * dx].setUrl("red.gif");
            }
        }
        else
        {
            ...
            for (i = 1; i <= 5; i++)
            {
               //amobaView.cells[yKezdo + (i - 1) * dy][xKezdo + (i - 1) * dx].getUpFace().setImage(zoldGyoztesSzinek[i - 1]);
                amobaView.cells[yKezdo-1 + (i - 1) * dy][xKezdo-1 + (i - 1) * dx].setUrl("green.gif");
            }
        }

//        if ( amobaModell.amoba.palya.Get( amobaModell.amoba.xGyoztes, amobaModell.amoba.yGyoztes) == TSzin.KEK )
//        {
//            lblAllasZold.setText( "" + ++amobaModell.zoldPontjai );
//        }
//        else
...
//        for ( k = 1; k <= 5; k++ )
//        {
//            amobaView.cells[yKezdo+(k-1)*dy][xKezdo+(k-1)*dx].setEnabled(true);
//        }
        ...
        for (int i = 0; i < amobaView.cells.length; i++)
        {
            ...
            {
                if ( amobaModell.amoba.palya.Get( j, i) != TSzin.URES )
                {
                    //amobaView.cells[i][j].getUpFace().setImage( amobaView.imgs.uresKor().createImage() );
                    amobaView.cells[i-1][j-1].setUrl("ures.png");
                    //amobaView.cells[i][j].setEnabled(true);
                }
